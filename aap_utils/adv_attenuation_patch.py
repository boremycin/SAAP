...
class AAPatch(EvasionAttack):
    ...
    def __init__(
        self,
        estimator: "OBJECT_DETECTOR_TYPE", # type: ignore
    ...
    def generate(  # type: ignore
        self, x: np.ndarray, y: Optional[List[Dict[str, np.ndarray]]] = None, 
        **kwargs,
    ) -> np.ndarray:
        """
        :param x: Sample images.
        :param y: Target labels for object detector.
        :return: Adversarial attenuation patch.
        """
        ...
        for i_step in trange(self.max_iter, desc="AdvAttenuationPatch iteration", disable=not self.verbose, position=1, leave=False):
            ...
            for e_step in range(self.sample_size):
                ...
                for i_batch in range(num_batches):
                    ...
                    patch_gradients = patch_gradients_old + np.sum(gradients, axis=0) #执行了叠加操作
                    logger.debug(
            ...
            update = np.sign(patch_gradients)
            update = np.nan_to_num(update, nan=0.0)
            self._patch = self._patch + update * (1 - 2 * int(self.targeted)) * self.learning_rate
            
            grayscale =  cv2.cvtColor(self._patch.transpose(1,2,0), cv2.COLOR_RGB2GRAY)
            self._patch = np.stack((grayscale, grayscale, grayscale), axis=0)
            ...
            target_mean = self.energy_tau*raw_area_energy 
    ...
    def _augment_images_with_patch(
        ...
        x_patch = x.copy().astype(np.float32)
...
##############################################################################################################################
        ...
        #x_patch[:, x_1:x_2, y_1:y_2, :] = patch_local # original replace method 
        #utilize decay way
